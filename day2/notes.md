...
** In MongoDB, a cursor is a pointer to the result set of a query. When you run a query, MongoDB does not return all the documents at once; instead, it returns a cursor that allows you to iterate over the results one by one.

###

** How Cursor Works
When you run a query like db.collection.find(), MongoDB does not return all matching documents immediately.
Instead, it returns a cursor object, which you can loop through to get the documents.
The cursor fetches documents in batches (default batch size is 101 documents).
You can use cursor methods like .next(), .forEach(), or .toArray() to process the data.
...
.sort({field: 1}) → Sorts the documents (1 for ascending, -1 for descending).
...
const count = db.comments.find({ type: "fruit" }).count();
...
$expr is used in MongoDB queries when you need to compare fields within the same document or perform calculations inside a find() query.

Normally, MongoDB's find() method only allows you to compare a field with a fixed value. But what if you need to compare two fields within the same document? That's when $expr is needed.
...
### Example: Find products where price is greater than cost.
Use Aggregation Operators in find()

Example: Calculate total price (quantity * unitPrice) and filter results.
Perform Logical or Arithmetic Operations Inside a Query

Example: Check if a user’s age is twice their experience.


Normal Query vs $expr Query
1️⃣ Without $expr (Only Compare with a Fixed Value)
...
### *** More Examples of $expr
✅ 1. Find Employees Where salary > bonus + 5000


```
db.employees.find({
  $expr: { $gt: ["$salary", { $add: ["$bonus", 5000] }] }
});
